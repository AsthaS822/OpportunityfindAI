...
class MultiSearch:
    ...
    def _build_strategies(self, intent: Dict, base: Dict) -> List[tuple]:
        ...
        if intent_type == "Career Guidance":
            strategies.append(("career_scholarships", {**base, "categories": ["scholarship", "fellowship"], **({"keywords": base.get("field", "")} if base.get("field") else {})}))
            strategies.append(("career_internships", {**base, "categories": ["internship", "job"], "keywords": f"{base.get('keywords', '')} {base.get('field', '')}"}))
            if not country or country == "india":
                strategies.append(("govt_schemes", {**base, "categories": ["scheme"], "keywords": "government scheme india"}))
        elif intent_type == "Government Scheme":
