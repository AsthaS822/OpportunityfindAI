...
class QueryParser:
    ...
    def parse(self, query: str, session_profile: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Structured intent: intent={intent_obj['intent']}, country={intent_obj['country']}, degree={intent_obj['degree']}")
        return intent_obj
    ...
    def _detect_study_abroad(self, q: str, session: Dict) -> bool:
        if any(w in q for w in ["abroad", "overseas", "international", "foreign", "study abroad"]):
            return True
        if session.get("study_abroad"):
            return True
        country = self._extract_single_country(q) or session.get("country")
        return bool(country and country not in ("India",))
